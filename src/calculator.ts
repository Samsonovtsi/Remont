...
function calculateSmetaEstimate(input: EstimateRequest): EstimateResult {
  ...
  const conditionAdjustment = conditionBaseWorks * (conditionFactor - 1);
  ...
  if (Math.abs(conditionAdjustment) >= 1) {
    ...
    const conditionTitle =
      conditionAdjustment > 0
        ? 'Доплата за состояние: ' + conditionLabel
        : 'Снижение стоимости за состояние: ' + conditionLabel;
...
export function calculateEstimate(input: EstimateRequest): EstimateResult {
  // Приоритет расчёта — фактические загруженные сметы.
  // В quick-режиме объёмы оцениваются автоматически, но ставки и структура
  // стоимости берутся из реальных смет. Техкарта остаётся справочным ориентиром.
